...
Patch14: parted-2.1-Document-align-check-642476.patch
# Default to 1MiB partition alignment
Patch15: parted-2.1-default-to-1MiB-alignment-when-possible.patch
# Double buffer for fdasd_check_api_version
Patch16: parted-2.3-double-buffer-for-fdasd_check_api_version-693852.patch
# Disable tests that fail when run as root
Patch17: parted-2.1-disable-root-tests.patch
# Add legacy_boot flag to GPT disklabel
Patch18: parted-2.3-gpt-add-legacy_boot-flag.patch
Patch19: parted-2.3-gpt-add-partition-attr-bits.patch
# Remove the asserts from CHS guessing code
Patch20: parted-2.4-remove-assert-from-dos-geometry.patch
# Fix the snap radius to use 1/2 unit range and don't allow values less than 1
Patch21: fix-snap-radius-so-that-it-is-using-half-665496.patch
Patch22: don-t-allow-values-less-than-1-665496.patch
Patch23: add-tests-for-snap-fixes.patch
# Fix a scsi_debug race condition that causes tests to fail
Patch24: fix-scsi_debug-race.patch
# Add a wait loop when removing the partition in t9030
Patch25: t9030-wait-for-partition-to-be-removed.patch
# Disable t9020
Patch26: disable-t9020.patch
# copy flags when duplicating GPT partitions and test
Patch27: parted-3.0-libparted-copy-flags-when-duplicating-GPT-partitions.patch
...
* Thu Oct 20 2011 Brian C. Lane <bcl@redhat.com> 2.1-17
...
* Tue Sep 27 2011 Brian C. Lane <bcl@redhat.com> 2.1-16
- Set the size of the test disk for t9020 which fails TPS on s390x
  Related: #746098

* Tue Sep 27 2011 Brian C. Lane <bcl@redhat.com> 2.1-15
- Add missing patch for scsi_debug race fix in tests
  Related: #665496

* Tue Jun 21 2011 Brian C. Lane <bcl@redhat.com> 2.1-14
- Fix scsi_debug race condition in tests
- turn t9020 and t9030 tests back on
- Add legacy_boot support to GPT disklabel (#711148)
  Resolves: #711148
- Remove asserts from CHS guessing code (#692562)
  Resolves: #692562
- Fix snap radius and don't allow values < 1 (#665496)
  Resolves: #665496

* Thu Apr 07 2011 Brian C. Lane <bcl@redhat.com> 2.1-13
- Double the buffer for fdasd_check_api_version
  Resolves: #693852
- Disable tests that fail when run as root

* Mon Dec 20 2010 Brian C. Lane <bcl@redhat.com> 2.1-11
- Document the align-check command
  Resolves: #642476
- Default to 1MiB partition alignment
  Resolves: #618255
